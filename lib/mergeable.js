...
const {
  GithubRateLimitRemainingTotal,
  GithubRateLimitLimitTotal,
} = require("./stats/githubPrometheusStats");
...
function logEventReceived(context) {
  ...
  if (event.includes("installation")) {
    ...
    if (event === "installation") {
      repositoriesAdded = context.payload.repositories.map(
        (repo) => repo.full_name
      );
    } else if (event === "installation_repositories") {
      repositoriesAdded = context.payload.repositories_added.map(
        (repo) => repo.full_name
      );
      repositoriesRemoved = context.payload.repositories_removed.map(
        (repo) => repo.full_name
      );
    }
...
class Mergeable {
  ...
  start(robot) {
    ...
    if (this.mode === "development") {
      ...
      scheduleIntervalSeconds =
        process.env.MERGEABLE_SCHEDULER_INTERVAL || 3600;
    }

    if (process.env.MERGEABLE_SCHEDULER === "true") {
      log.info(
        "Starting scheduler at " +
          scheduleIntervalSeconds +
          " second intervals."
      );
  ...
  flex(robot) {
    console.log("setting up flex executor");
